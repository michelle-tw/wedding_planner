...
import { pushNow } from '../lib/syncManager';
...
export function SyncSettings() {
  ...
  return (
    <Card className="space-y-4">
      ...
      <p className="text-sm text-ink-soft">{t('settings.sync.publicDesc')}</p>

      <div className="flex flex-wrap items-center gap-2 text-sm">
        {status === 'connecting' && <Loader2 size={15} className="animate-spin text-gold-500" />}
        {(status === 'synced' || status === 'off') && <Check size={15} className="text-sage-600" />}
      ...
      <div className="space-y-1.5">
        <label className="text-xs text-ink-soft">{t('settings.sync.appLinkLabel')}</label>
